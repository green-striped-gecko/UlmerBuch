...
```{r}
BAdata <- read.csv2("extdata/Burgaue_Larven.csv")
head(BAdata)
```
Das Kontrollgewässer wurde hier als "Control1", das Einflussgewässer als "Treatment" benannt. Als Zeit gilt 2023 als "Bevore" und 2024 als "After".
...
```{r}
ggplot(BAdata, aes(x = Group, y = LV_larvae_Numbers_of_Individual, fill = Time, group = interaction(Group, Time))) +
  geom_boxplot() +
  labs(
    x = "",
    y = "",
...
l.em
```
Während 2023 im Treatment Gewässer weniger Kammmolchlarven als im Kontrollgewässer waren (mit nur sehr geringem Unterschied), befanden sich nach der Maßnahme im Treatmentgewässer deutlich mehr Kammmolchlarven, im Kontrollgewässer deutlich weniger als im Vorjahr. 

### Graphische Darstellung
```{r}
ggplot(BAdata, aes(x = Group, y = TC_larvae_Numbers_of_Individual, fill = Time, group = interaction(Group, Time))) +
  geom_boxplot() +
  labs(
    x = "",
    y = "",
