---
title: "Onlinebeispiel 5.6 Erfassung von Froschrufen zur Unterstützung des Managements eines Naturschutzgebietes"
...
Diese Daten wurden als Datenframe eingeladen, wir benötigen aber Matritzen. Daher müssen wir sie umformatieren. Zunächst entfernen wir jedoch ein Gewässer, welches im Mai bereits ausgetrocknet war: Gewässer 15.
...
```{r}
# dabei wird Zeile 1 (ohne Wassertemperatur) ausgelassen
...
# für Zeit b
{boxplot(RBU.data, xlab = "Erfassung", ylab = "Häufigkeitsklasse", yaxt="n")
...
# für Temperatur
{plot(RBU$X1~WTemp$X1, yaxt="n",
      xlab = "Wassertemperatur [°C]",
      ylab = "Häufigkeitsklasse")
 axis(2, at = c(0, 1, 2, 3), labels = c("0","1","2","3")) 
}
...
# für Zeit b
{boxplot(LF.data, xlab = "Erfassung", ylab = "Häufigkeitsklasse", yaxt="n")
 axis(2, at = c(0, 1, 2, 3), labels = c("0","1","2","3")) 
}
...
### Modellvergleiche
Nutzen wir die Minima der negativen Log-Likelihood und korrigieren diese basierend auf der Anzahl Parameter, um den AIC zu berechnen (siehe Kapitel 9.3 des Buches) und die Modelle miteinander zu vergleichen:
...
```
Wir sehen, dass die Modelle 3 und 4 einen niedrigeren AIC Wert haben als 1 und 2. Die PIM2 (*p2* und *p3* verschieden von *p1*) hat also eine etwas bessere Modellgüte gezeigt. Es würde sich sicher lohnen, weitere Kombinationen miteinander zu vergleichen.
...
Nun können wir die erste Funktion `cp` nutzen, um aus den Schätzwerten (estimates) die tatsächlichen Belegungswahrscheinlichkeiten (*psi*) pro Häufigkeitsklasse zu berechnen.
```{r}
...
Die mittlere HK, die zu erwarten ist (= geschätzte mittlere latente HK), berechnet sich als *N* = 3 - 3 x *psi0* - 2 x *psi1* + *psi2*.
```{r}
