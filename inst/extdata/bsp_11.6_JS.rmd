---
title: "Onlinebeispiel 11.6. Vergleich verschiedener JS-Modelle zur Schätzung der Überlebensraten von Geckos"
...
## Fangstatistiken:
Bei *k* = 5 Erfassungsjahren reicht *i* von 1 bis 5.

$n_i$:	Gesamtzahl aller Tiere, die bei der *i*-ten Erfassung gefangen/festgestellt wurden (*i* = 1,...,*k*) ($n_i = m_i + u_i$);
...
$R_i$:	Anzahl Exemplare aus $n_i$, die nach der *i*-ten Erfassung (*i* = 1,...,*k*) wieder freigelassen wurden. Wenn ein Teil der zum Zeitpunkt *i* gefangenen Exemplare einbehalten wird (Belegentnahme, Verletzung bei Markierung etc.), können zum Zeitpunkt *i* nur noch $R_i$ < $n_i$ Exemplare freigelassen werden; in diesem Datensatz haben wir alle gefangenen Geckos wieder freigelassen, daher ist $R_i = n_i$.
...
$m_i$:	Anzahl markierter Individuen, die bei der *i*-ten Erfassung gefangen/festgestellt wurden (*i* = 1,...,*k*; $m_1$ = 0);
...
$m_{ij}$: Anzahl der im Jahr *i* markierten Individuen, die im Jahr *j* wiedergefangen wurden;

```{r}
# Matrix mij für den Wiederfang
# mit k-1 Zeilen und k Spalten
mij <- as.data.frame(matrix(nrow=k-1,ncol=k))
colnames(mij) <- 1:5
rownames(mij) <- 1:4
#mij
mij$"1" <- 0
...
```
Damit haben wir sowohl den Gesamtvektor $m_i$, als auch die Teilwerte für jede Fanggelegenheit *j*. Es ergibt sich

$r_i$:	Anzahl Individuen von $R_i$, die zum Zeitpunkt *i* freigelassen und bei einem späteren Fangtermin wiedergefangen wurden (*i* = 1,...,*k*-1);
...
$z_i$:	Anzahl Tiere, die vor der Fanggelegenheit *i* markiert und bei Fanggelegenheit *i* nicht, jedoch bei einer späteren Fanggelegenheit wiedergefangen wurden (*i* = 2,...,*k*-1).
...
$u_i$:	Anzahl unmarkierter Exemplare, die bei der *i*-ten Erfassung gefangen wurden (*i* = 1,...,*k*);
...
$M_i$:	Anzahl markierter Tiere, die sich zum Zeitpunkt der *i*-ten Erfassung in der Population befinden (*i* = 1,...,*k*; $M_1$ = 0);
...
$N_i$:	Gesamtzahl Individuen, die sich zum Zeitpunkt der *i*-ten Erfassung in der Population befinden (*i* = 1,...,*k*);
...
$phi_i$:	Überlebenswahrscheinlichkeit aller Tiere zwischen der *i*-ten und (*i*+1)-ten Erfassung (*i* = 1,...,*k*-1);
...
sePhii
```
Die Überlebensraten betragen von 2012 nach 2013 69,6 %, von 2013 nach 2014 62,3 % und von 2014 nach 2015 58 %; die Standardfehler sind sehr gering.

```{r}
# Gesamtvarianz der Überlebensrate
# d. h., die Varianz des Überlebensprozesses + die durch die Erfassung verursachte Varianz
GesVarPhi <- VarPhii + (phii*(1-phii))/(Mi-mi+Ri)
GesVarPhi
```

$B_i$:	Gesamtzahl der Exemplare, die zwischen der *i*-ten und (*i*+1)-ten Erfassung zur Population neu hinzugekommen sind und zum Zeitpunkt der (*i*+1)-ten Erfassung sich noch in der Population befinden (*i* = 1,...,*k*-1);
...
$q_i$:	Sterberate zwischen der *i*-ten und (*i*+1)-ten Erfassung (*i* = 1,...,*k*-1) ($q_i$ = 1 - $phi_i$);
...
$p_i$:	Fangwahrscheinlichkeit für alle Tiere, die sich bei der *i*-ten Erfassung in der Population befinden (*i* = 1,...,*k*).
```{r}
pi = mi/Mi
pi
```
Die Fangwahrscheinlichkeiten betragen zwischen 77 % in 2013 und 95,5 % in 2014.
...
Die derzeit umfassendste Software für die Analyse von Fang-Wiederfang-Daten ist MARK (White & Burnham 1999). MARK ist ein FORTRAN-Programm zur Anpassung von Fang-Wiederfang-Modellen, die manuell über eine grafische Benutzeroberfläche erstellt werden. RMark (Laake & Rexstad 2008) ist ein R-Paket, das Modelle für MARK mit benutzerdefinierten Formeln konstruiert, um die manuelle Modellerstellung zu ersetzen. Mit RMark und MARK können die meisten derzeit verfügbaren Fang-Wiederfang-Modelle in R angepasst und bearbeitet werden.

Weitere R-Pakete für die Analyse von Fang-Wiederfang-Daten sind FSA (Ogle et al. 2023), Rcapture (Baillargeon & Rivest 2007), mra (McDonald et al. 2005), secr (Borchers & Efford 2008, Efford 2025), SPACECAP (Royle et al. 2009), BaSTA (Colchero, Jones & Rebke 2012) und marked (Laake et al. 2013). FSA beinhaltet das JS-Grundmodell analog zum Programm JOLLY. Rcapture passt geschlossene und offene Modelle in einem log-linearen Rahmen an. Das mra-Paket passt das Cormack-Jolly-Seber-Modell (CJS) und das geschlossene Huggins-Modell mit einem Regressionsansatz zur Modellspezifikation an. Die Pakete secr und SPACECAP ermöglichen eine räumlich explizite Modellierung geschlossener Fang-Wiederfang-Daten, können aber auf offene Populationen erweitert werden (Bsp. auch Paket openCR, Efford 2024). BaSTA schätzt das Überleben mit Kovariaten aus Wiederfang-/Wiederfindungsdaten in einem Bayes'schen Rahmen, wenn viele Individuen ein unbekanntes Alter haben. Spezifikationen für das marked finden sich unten im entsprechenden Kapitel.

In diesem Beispiel konzentrieren wir uns auf die Pakete FSA, marked und RMark. Diese laden wir hier zunächst ein. Außerdem laden wir ein Paket für Goodness-of-fit-Tests ein: R2ucare. Und schließlich laden wir ggplot2 (Wickham 2016) und scales (Wickham et al. 2023) zur grafischen Darstellung der Ergebnisse.
...
# benoetigte R pakete
pakete <- c("FSA", "marked", "RMark", "R2ucare", "ggplot2", "scales")
...
\url{https://search.r-project.org/CRAN/refmans/FSA/html/mrOpen.html}

\url{https://cran.r-project.org/web/packages/FSA/FSA.pdf}

\url{https://cran.r-project.org/web/packages/marked/vignettes/markedVignette.html}

\url{https://cran.r-project.org/web/packages/R2ucare/vignettes/vignette_R2ucare.html}
...
head(ch1)
```
Hier bekommen wir eine Zusammenfassung der Fanggeschichten. Wir sehen nacheinander die wichtigsten Fangstatistiken:

`caphist` --- die zusammengefasste Fanggeschichten

`methodB.top` --- die $m_{ij}$-Matrix

`methodB.bot` --- die Fangstatistiken $m_i$, $u_i$, $n_i$ und $R_i$

`m.array` ---  die erste Spalte enthält $n_i$, die folgenden ($c_i$) die Wiederfänge bei *i* = 2, 3..., sowie 1-$r_i$. Die Spaltensummen ergeben $m_i$.
...
Das R-Paket marked (Laake et al. 2013) wurde für Analysen mit markierten Tieren entwickelt, als Gegensatz zum R-Paket unmarked (Fiske & Chandler 2011), welches wir für Occupancy-Modelle vorstellen (Beispiele 4.1 und 4.2). Die Entwickler des Paketes marked schreiben: Der ursprüngliche Anstoß für das Paket war die Implementierung des CJS-Modells unter Verwendung der von Pledger, Pollock & Norris (2003) beschriebenen hierarchischen Likelihood-Konstruktion und die Verbesserung der Ausführungszeiten mit RMark/MARK (White & Burnham 1999; Laake & Rexstad 2008) für die Analyse von großen Datensätze mit vielen zeitlich variierenden individuellen (tierbezogenen) Kovariablen. Anschließend wurde das Jolly-Seber-Modell mit der POPAN-Struktur von Schwarz & Arnason (1996) implementiert, wobei die Idee der hierarchischen Wahrscheinlichkeitskonstruktion auf den Eintritt von Tieren in die Population (Immigration) ausgedehnt wurde, was dem Konzept der Superpopulation entspricht. Zudem wurde eine Bayes'sche Markov-Chain-Monte-Carlo-Implementierung (MCMC) des CJS-Modells hinzugefügt, die auf dem von Albert & Chib (1993) verwendeten Ansatz zur Analyse binärer Daten mit einem Probit-Regressionsmodell basiert.
...
*phi(t)* (= $phi_i$) --- Überlebensrate (apparent survival)

*p(t)* (= $p_i$) --- Fängigkeit zum Zeitpunkt *t*
...
*pent(t)* (= $B_i$) --- Wahrscheinlichkeit, dass zum Zeitpunkt t neue Individuen aus der Superpopulation hinzukommen
...
gecko.js.predicted
```
Die (konstante) Überlebenswahrscheinlichkeit liegt damit bei 63,7 %, die (annähernd) konstante Fangwahrscheinlichkeit bei 87,7 % für Männchen und 90,4 % für Weibchen, die Eintrittswahrscheinlichkeit *pent* zwischen 10,2 % und 25 % und die Anzahl nicht markierter Individuen liegt bei 3, die Superpopulationsgröße liegt damit bei 145 + 3 = 148. 
...
names(ddl$p)
```
Da dies sehr complexe Modelle sind, nutzen wir die MCMC CJS-Modelle (`probitCJS`) - was wir bereits in den prozessierten Daten unter `model="probitCJS"` spezifiziert haben. Die Argumente `burnin` und `iter` kontrollieren hierbeit die Anzahl der Burnin-Iterationen und die Anzahl der Iterationen nach dem Burnin-Prozess.
```{r}
...
#head(reals)
```
Stellen wir diese einmal grafisch dar (wir benötigen dazu das Paket ggplot2):
```{r}
# Modell ohne MCMC
ggplot(reals$Phi,aes(Tsummer,estimate.mode,
        ...
        scale_y_continuous(labels = label_number(decimal.mark = ",")) +
	      xlab("\nSummer Temperature")+ylab("Überlebensrate\n")+facet_grid(residency~.)   

ggplot(reals$Phi,aes(svl,estimate.mode,
                     ymin=estimate.CI.lower,ymax=estimate.CI.upper))+
        geom_errorbar(width=0.2)+geom_point()+geom_line()+
        scale_y_continuous(labels = label_number(decimal.mark = ",")) +
        xlab("\nKopf-Rumpf-Länge")+ylab("Überlebensrate\n")+facet_grid(sex~.)   

ggplot(reals$Phi,aes(mass,estimate.mode,
                     ymin=estimate.CI.lower,ymax=estimate.CI.upper))+
        geom_errorbar(width=0.2)+geom_point()+geom_line()+
        scale_y_continuous(labels = label_number(decimal.mark = ",")) +
        xlab("\nMasse")+ylab("Überlebensrate\n")+facet_grid(sex~.) 

ggplot(reals$p[order(reals$p$catchability_mean),],aes(catchability_mean,estimate.mode,
                     ymin=estimate.CI.lower,ymax=estimate.CI.upper))+
        geom_errorbar(width=0.2)+geom_point()+geom_line()+
        scale_y_continuous(labels = label_number(decimal.mark = ",")) +
        xlab("\nJährlicher Wiederfang")+ylab("Fängigkeit\n")+facet_grid(sex~.)   

```

# CJS-Modelle mit dem R-Paket RMark
RMark ist eine Sammlung, die analog zum Programm MARK entworfen wurde. Lesern wird hier nahegelegt, für eine detaillierte Beschreibung in das MARK Handbuch (\url{http://www.phidot.org/software/mark/docs/book/}) sowie in das Benutzerhandbuch (User Guide) von RMark (\url{https://cran.r-project.org/web/packages/RMark/RMark.pdf}) zu schauen. 
...
Wir bleiben der Einfachheit halber bei unserem ersten Geckobeispiel, `gecko2`. Die Datenprozessierung muss noch einmal mit diesem Paket durchgeführt werden. Außerden nutzen wir nur das zuvor erstbeste Modell, welches `Phi(1)p(sex)pent(time)N(1)` war.
...
```
Die verbundenen schwarzen Punkte sind die Schätzungen aus dem Popan-Modell und deren 95 %-Konfidenzintervalle, die roten  Punkte sind die Schätzungen aus dem JS-Modell. Die Werte sind sehr ähnlich jedoch nicht identisch. Dies liegt an geringen Abweichungen in den ursprünglichen Schätzwerten. Während sich die Werte für *phi* (ca. 0,64), *pent* (0.25, 0.15, 0.19, 0.10) und *p* (Weibchen: 0.90, Männchen: 0.88) praktisch nicht unterscheiden, ist der Wert für *Nsuper* verschieden (148 vs. 176). Dies führt zu leicht unterschiedlichen Schätzungen der Populationsgrößen.

# Goodness-of-fit-Test (GoF) für CJS-Modelle
...
```
Keiner der Tests ist signifikant. Die Modellannahmen werden also nicht verletzt. Wenn doch ein Test signifikant wäre, beschreiben wir in Kap. 11.4 bis 11.10 des Buches, welche Lösungsmöglichkeiten es geben könnte. Entscheiden wir uns für ein Modell, welches beispielsweise Änderungen der Fangwahrscheinlichkeit oder Durchzügler berücksichtigt, wollen wir sicherlich im Anschluss prüfen, ob die Anpassung das Problem behoben hat. Die Gesamtteststatistik (`overall_CJS`) ist dabei die Summer der vier Einzeltests (Test.3Sr, Test3.Sm, Test2.Ct and Test.Cl). Der Test für Änderungen der Fangwahrscheinlichkeit wäre dann folgendermaßen zu berechnen:
...
\begin{hangparagraph}
Albert, J.H., Chib, S. (1993): Bayesian-analysis of binary and polychotomous response data. --- Journal of the American Statistical Association 88(422): 669–-79. 
\end{hangparagraph}

\begin{hangparagraph}
Baillargeon, S., Rivest, L.P. (2007): Rcapture: Loglinear models for capture-recapture in R. --- Journal of Statistical Software 19(5): 1–-31. 
\end{hangparagraph}
...
\begin{hangparagraph}
Colchero, F., Jones, O.R., Rebke, M. (2012): BaSTA: An R package for Bayesian estimation of age-specific survival from incomplete mark-recapture/recovery data with covariates. --- Methods in Ecology and Evolution 3: 466–-70. 
\end{hangparagraph}

\begin{hangparagraph}
Efford, M.G. (2024): openCR: Open population capture-recapture models. R package version 2.2.7. \url{https://CRAN.R-project.org/package=openCR/}
\end{hangparagraph}
...
\begin{hangparagraph}
Fiske, I.J., Chandler, R.B. (2011): unmarked: An R package for fitting hierarchical models of wildlife occurrence and abundance. --- Journal of
...
\begin{hangparagraph}
Grimm-Seyfarth, A., Mihoub, J.-B., Gruber, B., Henle, K. (2018): Some like it hot: From individual to population responses of an arboreal arid-zone gecko to local and distant climate. --- Ecological Monographs 88(3): 336–-352.
\end{hangparagraph}
...
\begin{hangparagraph}
Laake, J.L., Johnson, D.S., Conn, P.B. (2023): marked Package Vignette. \url{https://cran.r-project.org/web/packages/marked/vignettes/markedVignette.html}.
\end{hangparagraph}

\begin{hangparagraph}
McDonald, T.L., Amstrup, S.C., Regehr, E.V., Manly, B.F.J. (2005): Examples. --- S. 196–265 in: Amstrup, S.C., McDonald, T.L., Manly, B.F.J. (Hrsg.): --- Handbook of Capture-Recapture Analysis. Princeton University Press, Princeton, New Jersey USA.
\end{hangparagraph}

\begin{hangparagraph}
Ogle D.H., Doll, J.C., Wheeler, A.P., Dinno, A. (2023): FSA: Simple fisheries stock assessment methods. R package version 0.9.5, \url{https://CRAN.R-project.org/package=FSA}.
\end{hangparagraph}
...
\begin{hangparagraph}
Schwarz, C.J., Pickard, D., Marine, K., Bonner, S.J. (2009): Juvenile salmonid outmigrant monitoring evaluation, phase II, September 2009. --- Unpublished report prepared for the Trinity River Restoration Program, Weaverville, CA.
\end{hangparagraph}

\begin{hangparagraph}
Seber, G.A.F. (2002): The estimation of animal abundance. Second Edition. --- Edward Arnold (reprinted).
\end{hangparagraph}
...
\begin{hangparagraph}
Wickham, H., Pedersen, T., Seidel, D. (2023): scales: Scale functions for visualization. R package version 1.3.0, \url{https://CRAN.R-project.org/package=scales}.
\end{hangparagraph}
