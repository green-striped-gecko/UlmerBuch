---
title: "Onlinebeispiel 6.7. Vergleich verschiedener Abfang-Modelle zur Schätzung der Populationsgröße von Amphibienlarven"
...
Nun können wir daraus die Regressionsgerade zwischen der Anzahl nacheinander entnommenen Tiere pro Kescherdurchgang und deren kummulativer Summe, d. h. der Anzahl zu dem Zeitpunkt insgesamt entnommener Tiere, berechnen.
...
# Berechnung mittels removal Funktion im R-Paket FSA
```{r}
...
Das R-Paket FSA (Ogle et al. 2023) erlaubt verschiedene Removal-Methoden zur Schätzung der Populationsgröße. Anhand des Laubfroschs stellen wir die verschiedenen Methoden hier einmal vor. Wir wählen den Laubfrosch aus, da die Abfangdaten für den Laubfrosch vergleichsweise gut zwischen den Abfanggelegenheiten abnehmen. Bei den Molchen ist die Fängigkeit zu Beginn wahrscheinlich verschieden von den folgenden Fängigkeiten, was wir weiter unten besprechen werden.
...
Die von Burnham entwickelte Methode ist ein sogenannter genereller k-pass Schätzer / Likelihood-Methode. Dieser ist in der Windows Software MicroFish (Van Deventer und Platts 1985) implementiert. Auch diese Methode darf nur angewendet werden, wenn die Datenerfassung den drei Grundvorraussetzungen nach Zippin (1958) folgt. Ebenso treten ähnliche Probleme bei sehr geringen oder sehr variablen Fangwahrscheinlichkeiten oder nicht ausreichend abnehmender Fangzahl auf, was die Berechnung der Maximum-Likelihood unmöglich machen würde.
...
Dazu gibt es eine Anpassung von Moran's Likelihood-Methode: Die Likelihood-Methode von Schnute (1983) nutzt ein Modell, das eine unterschiedliche Fangwahrscheinlichkeit für das erste Abfangereignis, aber eine konstante Fangwahrscheinlichkeit für alle folgenden Abfangereignisse annimmt. 
...
\begin{hangparagraph}
Ogle D.H., Doll, J.C., Wheeler, A.P., Dinno, A. (2025): FSA: Simple fisheries stock assessment methods. R package version 0.9.6, \url{https://CRAN.R-project.org/package=FSA}
\end{hangparagraph}
...
\begin{hangparagraph}
Seber, G.A.F. (2002): The estimation of animal abundance. Second Edition. --- Edward Arnold (Reprint). 
\end{hangparagraph}

\begin{hangparagraph}
Van Deventer, J.S., Platts, W.S. (1983): Sampling and estimating fish populations from streams. --- Transactions of the 48th North American Wildlife and Natural Resource Conference 349--354. 
\end{hangparagraph}

\begin{hangparagraph}
Van Deventer, J.S., Platts, W.S. (1985): A computer software system for entering, managing, and analyzing fish capture data from streams. --- United States Department of Agriculture. Forest Service. Research Note INT-352.
\end{hangparagraph}
...
\begin{hangparagraph}
Wickham, H., Vaughan, D., Girlich, M. (2024): tidyr: Tidy messy data. R package version 1.3.1, \url{https://CRAN.R-project.org/package=tidyr}.
\end{hangparagraph}
